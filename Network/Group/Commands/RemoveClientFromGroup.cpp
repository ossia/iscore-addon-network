...
RemoveClientFromGroup::RemoveClientFromGroup(
    Id<Client> client,
    Id<Group> group)
    : m_client{client}, m_group{group}
{
}

void RemoveClientFromGroup::undo(const score::DocumentContext& ctx) const
{
  ctx.plugin<Network::NetworkDocumentPlugin>().groupManager().group(m_group)->addClient(m_client);
}

void RemoveClientFromGroup::redo(const score::DocumentContext& ctx) const
{
  ctx.plugin<Network::NetworkDocumentPlugin>().groupManager().group(m_group)->removeClient(m_client);
}
